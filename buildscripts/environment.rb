...
namespace :env do
  ...
  task :set_dirs do
    ...
    FOLDERS[:autotx][:out] = File.join(FOLDERS[:src], PROJECTS[:autotx][:dir], 'bin', CONFIGURATION)
    CLEAN.include(FOLDERS[:autotx][:out])

    # for tests
    FOLDERS[:autotx][:test_out] = File.join(FOLDERS[:src], PROJECTS[:autotx][:test_dir], 'bin', CONFIGURATION)
    FILES[:autotx][:test] = File.join(FOLDERS[:autotx][:test_out], "#{PROJECTS[:autotx][:test_dir]}.dll")
